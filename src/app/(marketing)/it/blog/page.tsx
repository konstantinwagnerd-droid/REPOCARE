...
export const metadata = { title: "Blog", alternates: { canonical: "/it/blog" } };
...
function getPosts() {
  const dir = path.join(process.cwd(), "content", "blog-it");
...
export default function ItBlogPage() {
  ...
  return (
    <div className="container py-20">
      ...
      <h1 className="font-serif text-5xl font-semibold">Approfondimenti per l&apos;assistenza moderna.</h1>
      <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {posts.map((p) => (
          <Card key={p.slug}><CardContent className="p-6">
            <Badge variant="outline">{p.category}</Badge>
            <h2 className="mt-3 font-serif text-xl font-semibold"><Link href={`/it/blog/${p.slug}`} className="hover:text-primary">{p.title}</Link></h2>
